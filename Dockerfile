...
RUN source /root/.bashrc && \
  ...
  (cd build-wasm && make VERBOSE=1 --trace -C deps -j 6 BUILDING_HOST_TOOLS=1 install-libuv install-utf8proc 2>&1 | tee log) && \
  (cd build-wasm && make VERBOSE=1 --trace -C deps -j 6 2>&1 | tee log) && \
  (cd build-native && make VERBOSE=1 --trace -j 6 2>&1 | tee log) && \
  (cd build-wasm && make VERBOSE=1 --trace -j 6 julia-ui-release 2>&1 | tee log)
